...
function createPlot(titleName, numberOfHouses) {
	...
	// load the data 
	d3.json(String(numberOfHouses) + "houses" + ".json", function(error, data) {
		...
		Object.keys(data).forEach(function(key) {
			...
			// object for each algorithm in array
			counts[key] = {};

			// for each value of algorithm
			...
			dataArray.push({"algorithm": key, "values": thisArray})
			thisArray = [];
	...
	*/
	function drawBarChart(data, valueRanges) {
		...
		color.domain(data.map(function(d) { return d.algorithm}))
		...
		// get the algorithms
		const algs = data.map(x => x.algorithm);

		// draw the bars
		d3.map(data, function(data) {
			chart.selectAll(".bar" + data.algorithm)
			.data(data.values)
			.enter().append("rect")
				.attr("class", "bar" + data.algorithm)
				.attr("y", function(d) { return y(d.percentage)})
				.attr("x", function(d) { 
					return x(d.range) + x.rangeBand() / algs.length * algs.indexOf(data.algorithm) - (width / 20 / 2)
				})
				.attr("width", x.rangeBand() / algs.length)
				.attr("height", function(d) { return height - y(d.percentage) })
				.style("fill", color(data.algorithm)) 
				.style("opacity", .8)
		});
	drawLegend(data, algs)
	};
